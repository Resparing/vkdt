...
vkdt::GPU::GPU::GPU(vkdt::instance::instance* vkdtInstance, const bool debug, const bool verbose) noexcept : debug(debug), verbose(verbose)
{
	...
	this -> pVVKInstance = const_cast<VkInstance*>(&vkdtInstance -> refVKInstance());
...
void vkdt::GPU::GPU::findVKDTGPU(const char* vkdtGPUName)
{
	...
	if(vkEnumeratePhysicalDevices(*this -> pVVKInstance, &vkAvailablePhysicalDeviceCount, nullptr) != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to Find Number of VKDT GPU's!\n");
	}
	...
	if(vkEnumeratePhysicalDevices(*this -> pVVKInstance, &vkAvailablePhysicalDeviceCount, vkAvailablePhysicalDevices.data()) != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to Find VKDT GPU's!\n");
	}
